...
contract WhitelistProvider is Context, AccessControl {
     ...
     */
    event AddressRemovedFromWhitelist(uint256 indexed ipoId, address indexed account);
    /**
     ...
     */
    event BatchAddressesRemovedFromWhitelist(uint256 indexed ipoId, address[] accounts);
    
    /**
     * @dev Restricts function access to admin role
     */
    modifier onlyAdmin() {
        require(hasRole(ADMIN_ROLE, _msgSender()), "WhitelistProvider: admin role required");
        _;
    }
    ...
    function addToWhitelist(uint256 ipoId, address account) external onlyAdmin {
        ...
        require(!whitelistedAddresses[ipoId][account], "WhitelistProvider: already whitelisted");
     ...
     */
    function removeFromWhitelist(uint256 ipoId, address account) external onlyAdmin {
        require(account != address(0), "WhitelistProvider: invalid address");
        require(whitelistedAddresses[ipoId][account], "WhitelistProvider: not whitelisted");
     ...
     */
    function batchAddToWhitelist(uint256 ipoId, address[] calldata accounts) external onlyAdmin {
     ...
     */
    function batchRemoveFromWhitelist(uint256 ipoId, address[] calldata accounts) external onlyAdmin {
        for (uint256 i = 0; i < accounts.length; i++) {
            address account = accounts[i];
            if (account != address(0) && whitelistedAddresses[ipoId][account]) {
                whitelistedAddresses[ipoId][account] = false;
     ...
     */
    function isAllowed(uint256 ipoId, address account) 
        external 
        view 
        returns (bool) 
    {
     ...
     */
    function getWhitelistedAddresses(uint256 ipoId) external view returns (address[] memory) {
     ...
     * @return True if address is whitelisted
     */
    function isWhitelisted(uint256 ipoId, address account) external view returns (bool) {
        return whitelistedAddresses[ipoId][account];
    }
